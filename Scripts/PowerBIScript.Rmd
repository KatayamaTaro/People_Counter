...
```{r}
# Set file path for data input. When updating with new data, just update the All_20250507_XXXX.csv file path

base_dir<- ".."

file_path <- file.path( "Data", "Raw", "All_20250507_20260512.csv")
...
mutate(
      PedestriansIn = coalesce(PedestriansIn, 0),
      PedestriansOut = coalesce(PedestriansOut, 0),
      
      # UPDATED: Primary count now uses the average of In and Out
      TotalCount = (PedestriansIn + PedestriansOut) / 2,
      
      # Combined count remains the raw sum
 ...
 #Returns a processed data data set called processed_data
...
create_powerbi_main <- function(data) {
  ...
  powerbi_main <- data %>%
    ...
    mutate(
      # Power BI date formats (creates new columns)
...
export_powerbi_main <- function(dataset, base_path = file.path("..", "Data", "Processed", "PowerBI")) {
  ...
  output_file <- file.path(base_path, "EcoCounter_PowerBI_Main2.csv")
...
final_powerbi_data <- export_powerbi_main(dataset= powerbi_main, 
                                          base_path = file.path("Data", "Processed", "PowerBI", ""))
